...
export default function StaffPage() {
  ...
  async function setupCredentials() {
    ...
    const { data: existing } = await supabase.from('users').select('id').eq('username', credForm.username.toLowerCase()).neq('id', credModal.id).maybeSingle()
    ...
    const { data: authData, error: authError } = await supabase.auth.signUp({ email: authEmail, password: credForm.password, options: { data: { full_name: credModal.full_name } } })
    if (authError) { setCredError(authError.message); setSaving(false); return }

    await supabase.from('users').update({ auth_id: authData.user?.id, email: authEmail, username: credForm.username.toLowerCase() }).eq('id', credModal.id)
          ...
          <div key={s.id} className={`card ${!s.is_active ? 'opacity-60' : ''}`}>
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 rounded-full bg-brand-100 flex items-center justify-center text-brand-700 font-bold flex-shrink-0">{s.full_name[0].toUpperCase()}</div>
            ...
            <div className="flex gap-2 mt-3 pt-3 border-t border-surface-100 flex-wrap">
              <button onClick={() => { setCredModal(s); setCredForm({ username: s.username || '', password: '', confirmPassword: '' }); setCredError(''); setCredSuccess('') }}
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-panel max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-ink-400 mt-1">{form.permissions.length === 0 ? 'Full access to everything' : `${form.permissions.length} pages selected`}</p>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-panel">
            ...
            <div className="p-5 space-y-4">
              ...
              <div><label className="label">Password</label><input type="password" className="input" value={credForm.password} onChange={e => setCredForm(f => ({...f, password: e.target.value}))} placeholder="Minimum 6 characters" /></div>
              <div><label className="label">Confirm Password</label><input type="password" className="input" value={credForm.confirmPassword} onChange={e => setCredForm(f => ({...f, confirmPassword: e.target.value}))} /></div>
      ...
      {pwModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-panel p-5 space-y-4">
            <div className="flex items-center justify-between"><h3 className="font-semibold text-ink-900">Change My Password</h3><button onClick={() => setPwModal(false)} className="text-ink-300 text-xl">✕</button></div>
            <div className="bg-surface-50 rounded-xl p-3"><p className="text-sm font-semibold text-ink-900">{profile?.full_name}</p><p className="text-xs text-ink-400">@{profile?.username}</p></div>
            <div><label className="label">New Password</label><input type="password" className="input" value={pwForm.newPw} onChange={e => setPwForm(f => ({...f, newPw: e.target.value}))} placeholder="Minimum 6 characters" /></div>
            <div><label className="label">Confirm New Password</label><input type="password" className="input" value={pwForm.confirm} onChange={e => setPwForm(f => ({...f, confirm: e.target.value}))} /></div>
