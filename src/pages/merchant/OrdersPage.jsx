    ...
    <div className="space-y-5">
                ...
                <div key={order.id} className="p-4 hover:bg-surface-50 transition-colors">
                  ...
                  {(order.status === 'new' || order.status === 'assigned') && (
                    <div className="flex gap-2 mt-3 flex-wrap">
                      ...
                      <button onClick={() => setRescheduleModal(order)}
      ...
      {rescheduleModal && (
        <ScheduleFollowUpModal
          ...
          order={{ id: rescheduleModal.id, customerName: rescheduleModal.customers?.full_name }}
          scope="merchant"
          businessId={profile.business_id}
          actorId={profile.id}
          actorName={profile.full_name}
          defaultAssignedTo={rescheduleModal.assigned_cs_rep || (isScoped ? profile.id : null)}
          actionType="Call Customer"
          taskTitle={`Call ${rescheduleModal.customers?.full_name || 'customer'} — confirm on rescheduled date`}
          priority="normal"
          notifyRecipientId={
            (rescheduleModal.assigned_cs_rep || (isScoped ? profile.id : null)) !== profile.id
              ? (rescheduleModal.assigned_cs_rep || null)
              : null
          }
          notifyBusinessId={profile.business_id}
          helperText="This creates a follow-up task for the assigned rep and records it on the order timeline. Order stays as-is — it will not be sent to logistics until you confirm it."
          onClose={() => setRescheduleModal(null)}
          onSuccess={() => { setRescheduleModal(null); loadAll() }}
        />
      )}
