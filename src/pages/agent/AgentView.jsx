...
export default function AgentView() {
  ...
  return (
    <div className="min-h-screen bg-surface-50">
                      ...
                      <div className="flex gap-2 mt-4 flex-wrap">
                        ...
                        {d.status === 'out_for_delivery' && (
                          <>
                            ...
                            <button onClick={() => setRescheduleModal(d)} className="text-xs px-3 py-1.5 rounded-lg bg-amber-50 text-amber-700 font-medium">📅 Reschedule</button>
                          </>
                        )}
                      </div>
      ...
      {rescheduleModal && agent && (
        <ScheduleFollowUpModal
          heading="Log Reschedule"
          order={{ id: rescheduleModal.order_id, customerName: rescheduleModal.orders?.customers?.full_name }}
          scope="logistics"
          businessId={agent.logistics_id}
          actorId={profile.id}
          actorName={profile.full_name}
          defaultAssignedTo={profile.id}
          actionType="Attempt Redelivery"
          taskTitle={`Redeliver to ${rescheduleModal.orders?.customers?.full_name || 'customer'}`}
          notifyRecipientId={rescheduleModal.orders?.assigned_cs_rep}
          notifyBusinessId={rescheduleModal.orders?.merchant_id}
          helperText="This creates a redelivery follow-up and notifies the CS Rep."
          onClose={() => setRescheduleModal(null)}
          onSuccess={() => { setRescheduleModal(null); load() }}
        />
      )}
    </div>
  )
