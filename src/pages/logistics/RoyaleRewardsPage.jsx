...
export default function RoyaleRewardsPage() {
  ...
  const [ruleForm, setRuleForm] = useState({ name: '', targetType: 'role', applies_to_role: 'agent', applies_to_user_id: '', calculation_method: 'flat', rate_amount: '', rate_percent: '' })
  ...
  async function loadAll() {
    ...
    const [rwRes, agRes, ruRes] = await Promise.all([
      ...
      supabase.from('agents').select('id, user_id, users(id, full_name)').eq('logistics_id', bid),
  ...
  function resetRuleForm() {
    setRuleForm({ name: '', targetType: 'role', applies_to_role: 'agent', applies_to_user_id: '', calculation_method: 'flat', rate_amount: '', rate_percent: '' })
    setRuleError('')
  }

  async function saveRule() {
    if (!ruleForm.name) { setRuleError('Rule name is required.'); return }
    if (ruleForm.targetType === 'staff' && !ruleForm.applies_to_user_id) { setRuleError('Select an agent.'); return }
    ...
    await supabase.from('commission_rules').insert({
      ...
      name: ruleForm.name,
      applies_to_role: ruleForm.targetType === 'role' ? 'agent' : null,
  ...
  rewards.forEach(r => {
    ...
    if (!byAgent[name]) byAgent[name] = { earned: 0, paid: 0, count: 0 }
    byAgent[name].earned += Number(r.amount_earned)
    byAgent[name].paid += Number(r.amount_paid)
    byAgent[name].count += 1
  })
  const agentRows = Object.entries(byAgent).map(([name, v]) => ({ name, ...v })).sort((a, b) => b.earned - a.earned)

  function exportCSV() {
    exportRowsToCSV(rewards, [
      ['Agent', r => r.users?.full_name || ''],
      ...
      ['Status', r => r.status],
      ['Notes', r => r.notes || ''],
    ], `agent-commissions-${new Date().toISOString().split('T')[0]}.csv`)
  }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-ink-400 text-sm mt-0.5">Agent delivery commissions, earned automatically on successful delivery</p>
        </div>
      </div>
      ...
      {loading ? (
                ...
                {filtered.map(r => (
                  <div key={r.id} className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-semibold text-ink-900 text-sm">{r.users?.full_name || 'Unknown agent'}</p>
                        <p className="text-xs text-ink-400 mt-0.5">Delivered to: {r.orders?.customers?.full_name || '—'}</p>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-ink-400 mt-0.5">
                      Applies to: {r.applies_to_user_id ? agents.find(a => a.user_id === r.applies_to_user_id)?.users?.full_name || 'Specific agent' : (r.applies_to_role || 'All agents')}
                    </p>
                    <p className="text-xs text-brand-600 mt-0.5">
                      {r.calculation_method === 'flat' ? `Flat ${fmtNaira(r.rate_amount)} per delivery` : `${r.rate_percent}% of ${r.calculation_method === 'percent_order' ? 'order value' : 'gross profit'}`}
                    </p>
                  </div>
        ...
        <div className="space-y-5">
          ...
          <div className="card">
            <h2 className="font-semibold text-ink-900 mb-4">Top Performing Agents</h2>
            {agentRows.length === 0 ? <p className="text-sm text-ink-400 text-center py-6">No data yet.</p> : (
              <div className="space-y-3">
                {agentRows.map(a => (
                  <div key={a.name} className="flex items-center justify-between text-sm">
                    <span className="font-medium text-ink-700">{a.name}</span>
                    <div className="flex items-center gap-4 text-xs text-ink-500">
                      <span>{a.count} deliveries</span>
                      <span>Earned: <strong className="text-ink-900">{fmtNaira(a.earned)}</strong></span>
                      <span>Paid: <strong className="text-green-600">{fmtNaira(a.paid)}</strong></span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-panel max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              <div><label className="label">Rule Name</label><input className="input" value={ruleForm.name} onChange={e => setRuleForm(f => ({...f, name: e.target.value}))} placeholder="e.g. Agent Flat Delivery Commission" /></div>
              <div>
                <label className="label">Applies To</label>
                <div className="flex gap-2 mb-2">
                  <button type="button" onClick={() => setRuleForm(f => ({...f, targetType: 'role'}))} className={`flex-1 py-2 rounded-xl text-sm font-medium border ${ruleForm.targetType === 'role' ? 'bg-brand-600 text-white border-brand-600' : 'bg-white border-surface-200 text-ink-600'}`}>All Agents</button>
                  <button type="button" onClick={() => setRuleForm(f => ({...f, targetType: 'staff'}))} className={`flex-1 py-2 rounded-xl text-sm font-medium border ${ruleForm.targetType === 'staff' ? 'bg-brand-600 text-white border-brand-600' : 'bg-white border-surface-200 text-ink-600'}`}>Specific Agent</button>
                </div>
                {ruleForm.targetType === 'staff' && (
                  <select className="input" value={ruleForm.applies_to_user_id} onChange={e => setRuleForm(f => ({...f, applies_to_user_id: e.target.value}))}>
                    ...
                    {agents.map(a => <option key={a.id} value={a.user_id}>{a.users?.full_name}</option>)}
                  </select>
                )}
              </div>
              <div>
                ...
                <select className="input" value={ruleForm.calculation_method} onChange={e => setRuleForm(f => ({...f, calculation_method: e.target.value}))}>
                  <option value="flat">Flat amount per successful delivery</option>
                  <option value="percent_order">Percentage of order value</option>
                </select>
              </div>
              {ruleForm.calculation_method === 'flat' ? (
                <div><label className="label">Amount per delivery (₦)</label><input type="number" className="input" value={ruleForm.rate_amount} onChange={e => setRuleForm(f => ({...f, rate_amount: e.target.value}))} /></div>
              ) : (
      ...
      {reverseModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-panel p-5 space-y-4">
            ...
            <div><label className="label">Reason</label><textarea className="input" rows={3} value={reverseReason} onChange={e => setReverseReason(e.target.value)} placeholder="e.g. Delivery was disputed/reversed" /></div>
            <div className="flex gap-3">
