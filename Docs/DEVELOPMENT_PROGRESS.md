...
### ✅ B3. Tasks scheduling/reminder extension — COMPLETE. All five reschedule surfaces now run through one shared, conflict-protected system.
...
- A 5th uncoordinated entry point was then found in `TasksPage.jsx`'s outcome-logging flow (below) — missed by the original 3-then-4-surface framing entirely, since it's semantically an outcome being logged, not an obvious "reschedule."

**✅ What was implemented and deployed (schema live in Supabase; all files deployed and retested):**
...
- **`src/lib/schedulingHelpers.js`** — the one shared source of truth for every surface:
  ...
  - `scheduleFollowUp({...})` — the single write path for every surface, order-linked task-outcome flow included. Re-fetches the schedule immediately before writing and compares against what the caller last saw (`expectedSetAt`); returns `{ conflict: true, current }` without writing if something changed since the modal opened, instead of silently overwriting. Also owns `createMerchantTask()`/`createLogisticsTask()` (moved here from `taskHelpers.js` to avoid a circular import once `taskHelpers.js` needed to call back into this file — nothing else imported them directly, confirmed by grep before moving).
  - Closes the old task via `oldTaskId`/`oldTaskDisposition` (`'cancelled'` = superseded, used by the first four surfaces; `'completed'` with `oldTaskOutcomeFields` = outcome preserved, used by the 5th) — if the order's actual current pointer task differs from `oldTaskId`, **both** are closed correctly (the real one gets the caller's disposition, the stale pointer gets `'cancelled'`), so nothing is ever left dangling regardless of which surface acts.
  - `autoRetryOnConflict` (used only by the 5th surface, which has no interactive "keep/change" dialog): retries once against a freshly-seen schedule instead of surfacing a conflict, since completing an outcome is itself a deliberate scheduling decision.
  - Logs one consistent Timeline event type (`follow_up_scheduled`) and fires a notification, on every surface including the outcome flow (which previously logged nothing to Timeline at all — a small positive side effect of the consolidation).
- **`src/components/shared/ScheduleFollowUpModal.jsx`** — the one shared reschedule UI for the first four surfaces (the 5th, `TasksPage.jsx`, keeps its own outcome modal — see below).
- **`src/lib/taskHelpers.js`** — `completeTaskWithOutcome()`'s `RESCHEDULE_OUTCOMES` branch (the 5th surface) now calls `scheduleFollowUp()` for any order-linked task, passing `oldTaskDisposition: 'completed'` so the CS Rep's outcome/notes/next-action are preserved on the closed task rather than overwritten with a generic "superseded" note. A standalone task with no `order_id` (an internal reminder) keeps its original direct close-and-spawn behavior unchanged — there's no order to hold a pointer for. `rescheduleTask()` remains narrowed to that same standalone case only (refuses and logs an error if called on an order-linked task). Verified with `node --check` (real ESM syntax validation) in addition to the usual brace-balance check.
- **Wired into all five surfaces**, each now using the shared system:
  - `RequestsPage.jsx` (Royale) — `ScheduleFollowUpModal`, `scope: 'logistics'`, notifies the merchant's assigned CS Rep
  - `AgentView.jsx` — same, defaults the task to the agent themselves
  - `OrdersPage.jsx` (Merchant) — `scope: 'merchant'`, only notifies the assigned rep if it's someone other than the actor
  - `RoyaleTasksPage.jsx` — branches on `task.order_id`: order-linked tasks use `ScheduleFollowUpModal`; standalone tasks keep a simple direct-edit modal
  - `TasksPage.jsx` (Merchant) — **no file changes needed here at all**; its existing call to `completeTaskWithOutcome()` already passed everything required (`task.order_id`, `merchant_id`/`logistics_id`, `assigned_to`, `priority`, `title` via its `*` wildcard select), so routing it through the shared system was entirely contained inside `taskHelpers.js` + `schedulingHelpers.js`.

**🐛 Bug found and fixed during the 4-surface retest — worth remembering for future schema changes:** Royale's Follow-ups page came up completely empty after deploy (`No pending follow-ups`), even for tasks that definitely existed with correct data — confirmed via a direct SQL check before touching any code. Root cause: adding `orders.next_follow_up_task_id` (a new FK from `orders` to `tasks`) gave PostgREST **two** relationships between `tasks` and `orders`, so any query embedding `orders` inside a `tasks` select without explicitly naming which relationship to use now fails with `PGRST201`. Fixed by changing `orders(...)` to `orders!tasks_order_id_fkey(...)` in `RoyaleTasksPage.jsx` — the only place with this pattern *and* no explicit hint (`TasksPage.jsx` already had it, confirming the constraint name).
**Lesson for next time an FK is added between two tables that already have queries embedding one inside the other: grep the whole codebase for `.from('<table>')` selects embedding the other table, and add an explicit `!constraint_name` hint to each one** — this class of bug produces no console error by default unless the calling code explicitly logs `.error`.

**Retested and confirmed working, end to end, on all five surfaces:** basic reschedule creates the correct task on each of the first four; the "already scheduled" warning and the two-tab concurrency check both work as designed; "Needs Another Follow-Up"/"Re-delivery Scheduled" on `TasksPage.jsx` closes the old task as `completed` with its outcome intact (not "Superseded"), creates the new follow-up task, and logs the Timeline entry.

**Deferred items (unrelated to B3, unchanged, still open):**
- The three compact Dashboard summary widgets (CS/Royale/Merchant Due-Today/Tomorrow/Overdue counts on the main dashboard pages) — not built, unverified sandbox state for `MerchantDashboard.jsx`/`LogisticsDashboard.jsx`.
- Automatic Order Assignment, Cart Abandoned Tracking, Form Analytics, Mobile App View.
...
**✅ Step 4 complete — Bundle COGS/Finance behavior VERIFIED, zero code changes required:**
- Full repo search confirmed every COGS/profit calculation site (`financeRollupHelpers.js` ×2, `FinancePage.jsx`'s own all-time total, `ReportsPage.jsx`, `rewardsHelpers.js`, `marketingHelpers.js`) uses the identical generic formula `unit_cost_price × quantity` (or `(unit_selling_price − unit_cost_price) × quantity`), with no reference anywhere to `product_id`/`bundle_id`/`products`/`product_bundles` — bundles fit this architecture automatically because Step 1 already snapshots these fields correctly
